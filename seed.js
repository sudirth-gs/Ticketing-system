/**
 * Seed script: Populates Cosmos DB with initial mock data.
 * Run once with: node seed.js
 */
import { usersContainer, ticketsContainer, repliesContainer, initializeDatabase } from './cosmosDb.js';

const seedUsers = [
  { id: "cust-1", email: "customer@example.com", password: "customer123", name: "John Customer", role: "customer" },
  { id: "cust-2", email: "alice@example.com", password: "customer123", name: "Alice Miller", role: "customer" },
  { id: "admin-1", email: "admin", password: "admin123", name: "Sarah Support", role: "admin" }
];

const seedTickets = [
  {
    id: "TCK-1001", customerId: "cust-1", customerName: "John Customer",
    ...
    category: "Billing", priority: "High", status: "Open", createdAt: "2026-07-08T09:30:00Z"
  },
  {
    id: "TCK-1002", customerId: "cust-1", customerName: "John Customer",
    ...
    category: "Technical", priority: "Medium", status: "In Progress", createdAt: "2026-07-08T14:20:00Z"
  },
  {
    id: "TCK-1003", customerId: "cust-2", customerName: "Alice Miller",
    ...
    category: "Account", priority: "Low", status: "Resolved", createdAt: "2026-07-07T11:00:00Z"
  }
];

const seedReplies = [
  {
    id: "rep-1", ticketId: "TCK-1002", senderId: "cust-1", senderName: "John Customer", senderRole: "customer",
  ...
  {
    id: "rep-2", ticketId: "TCK-1002", senderId: "admin-1", senderName: "Sarah Support", senderRole: "admin",
  ...
  {
    id: "rep-3", ticketId: "TCK-1003", senderId: "admin-1", senderName: "Sarah Support", senderRole: "admin",
  ...
  {
    id: "rep-4", ticketId: "TCK-1003", senderId: "cust-2", senderName: "Alice Miller", senderRole: "customer",
...
async function seed() {
  try {
    // Step 1: Initialize DB and containers
    ...
    for (const user of seedUsers) {
      try {
        ...
        console.error(`  Error upserting user ${user.id}:`, err.message);
      }
    }
    ...
    for (const ticket of seedTickets) {
      try {
        ...
        console.log(`  Upserted ticket: ${ticket.id} - ${ticket.title}`);
      } catch (err) {
    ...
    for (const reply of seedReplies) {
      try {
        await repliesContainer.items.upsert(reply);
        console.log(`  Upserted reply: ${reply.id} (ticket: ${reply.ticketId})`);
      } catch (err) {
        console.error(`  Error upserting reply ${reply.id}:`, err.message);
      }
    }
